...
var allowed_tags = '<br /><br><ol></ol><ul></ul><li></li><strong></strong><i></i><h1></h1>';
var html5sticky = {};
var note_index = 0;
...
html5sticky.addNote = function(){
    ...
    var key = data.getKey();
    var stickynote = $('<div class="note_common ' + bgcolor + '"  data-id='+key+' />').appendTo($('#main'));
    ...
    if (Modernizr.localstorage){
       data.fset('notes/global/'+key , {
         head : null,
         date :  dated + '|' + html5sticky.getISODateTime(dateStr),
         bgColor : bgcolor,
         index : nindex,
...
html5sticky.saveNote = function(el){
   if (Modernizr.localstorage){
      ...
      const key = $('.bignote')[0].getAttribute('data-id');
      data.update('notes/global/'+key ,{
         heading : htext , 
         text : htext + '|' + ptext , 
     } )
...
html5sticky.getNotes = function(){
   if (Modernizr.localstorage){
          ...
          data.on('notes/global' , (snapshot)=>{
             ...
             // get position info
             temp_array = snapshot.val().pos.split('|');
             pleft = temp_array[0];
             ptop = temp_array[1];
 
             // get text info
             temp_array = snapshot.val().text.split('|');
             htext = temp_array[0];
             ptext = temp_array[1];
 
             
             stickynote = $('<div class="note_common ' + bgcolor + '"   data-id='+snapshot.val().key+' />').appendTo($('#main'));
             // add tape to stickynote
             html5sticky.addPin(stickynote);
             ...
             $(stickynote).append($('<p>Text here...</p>'));
             // append identifier
             $(stickynote).append($('<span id="idf_' + snapshot.val().key + 'stickynote' + '" />'));
...
$(function(){
   ...
   console.log(JSON.stringify(html5sticky))
